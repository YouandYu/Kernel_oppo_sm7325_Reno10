...
static bool is_same_app(u32 source_app_uid, u32 target_app_uid)
{
	return (source_app_uid % UID_MASK) == (target_app_uid % UID_MASK);
}
	...
	kuid_t *app_uid)
{
	...
	if (UNKNOW_APP_TYPE != get_app_type(ct)) {
...
static bool is_video_app(kuid_t app_uid)
{
	...
	for (i = 0; i < video_app_list.count; i++) {
		if (video_app_list.uid[i]) {
			...
			if (is_same_app(uid.val, app_uid.val)) {
				debug("oplus_sla video app, the uid = %d", app_uid.val);
				return true;
			}
		}
	}
	...
	enum ip_conntrack_info ctinfo)
{
	...
	if (INIT_APP_TYPE == get_app_type(ct)) {
		...
		for (i = 1; i < GAME_NUM; i++) {
			if (game_info[i].uid) {
				if (is_same_app(uid.val, game_info[i].uid)) {
					set_app_type(ct, i);
